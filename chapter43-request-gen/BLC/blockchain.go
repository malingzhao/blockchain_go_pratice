...
//初始化区块链
func CreateBlockChainWithGensisBlcok(address string, nodeId string) *BlockChain {
...
func (bc *BlockChain) AddBlock(block *Block) {
	err := bc.DB.Update(func(tx *bolt.Tx) error {
		...
		if nil != b {
			...
			lastestBlock := b.Get(blockHash)
			rawBlock := DeSerializeBlock(lastestBlock)
	...
	fmt.Printf("the new blocl is added")
}
...
//退出条件
func isBreakLoop(preBlockHash []byte) bool {
	var hashInt big.Int
	hashInt.SetBytes(preBlockHash)
	if hashInt.Cmp(big.NewInt(0)) == 0 {
		return true
	}
	return false
		...
		WorkOutLoop:
			for index, vout := range tx.Vouts {
				...
				if len(txInputs) > 0 {
					...
					if isSpent == false {
						//当给钱输出眉头被包含到txInputs中去
						txOutputs.TxOutputs = append(txOutputs.TxOutputs, vout)
					}
				} else {
					//没有input 使用该交易的输出 则代表当前交易中所有的输出都是utxo
					txOutputs.TxOutputs = append(txOutputs.TxOutputs, vout)
				}
			}
			utxoMaps[txHash] = txOutputs
			fmt.Printf("utxoMaps[txHash] :%v\n", utxoMaps[txHash])
		}
		if isBreakLoop(block.PrevBlockHash) {
			break
		}
